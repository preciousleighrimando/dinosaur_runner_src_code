...
def meduim_mode():
    ...
    while run:
        ...
        if not obstacles:
            if levels == 1:  # Level 1: Only small cacti
                choice = random.randint(0, 2)  # Corrected range to match the list length
                obstacles.append([SmallCactus, LargeCactus, BirdIndex][choice]([small_cactus, large_cactus, bird_img][choice]))
                obstacles[-1].rect.x += random.randint(100, 200)  # Adjust x-axis for proper spacing
                choice1 = random.randint(0, 5)  # Adjusted range for 1/6 chance
                if choice1 == 0:  # 1/6 chance to spawn no obstacle
                    pass
                else:
                ...
                if choice1 == 0:
                    ...
                    obstacles.append([SmallCactus, LargeCactus, BirdIndex][choice]([small_cactus, large_cactus, bird_img][choice]))
                    obstacles[-1].rect.x += random.randint(100, 200)  # Adjust x-axis for proper spacing
                    choice1 = random.randint(0, 2)  # Adjusted range for 1/6 chance
